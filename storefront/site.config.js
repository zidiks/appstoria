...
export const LEGAL = {
  ...
  address: 'г. Жлобин, м-н 17, дом 17',
  // Банковские реквизиты одной строкой: р/с, банк, БИК
  bank: "р/с BY93ALFA30122E18830010270000, в ЗАО 'АЛЬФА-БАНК' г.Минск, ул. Сурганова, 43-47. БИК ALFABY2X",
 ...
 */
// Регистр замены — как у найденного слова: Минск → Гродно, минск → гродно, МИНСК → ГРОДНО
const sameCase = (word, source) =>
  source === source.toUpperCase() && source.length > 1
    ? word.toUpperCase()
...
export const TEXT_REPLACEMENTS = [
  ...
  // Город: витрина — гродненская. «Гродно» не склоняется: Минска/Минску/Минске → Гродно
  [
    // «Минском» — существительное («под Минском»), но не прилагательное перед
    // словом на -е/-и («в минском офисе», «в Минском районе») — его берёт правило ниже
    new RegExp(
      `(?<![${RU_LETTER}])минск(?:а|у|е|ом(?!\\s+[${RU_LETTER}]*[еи](?![${RU_LETTER}])))?(?![${RU_LETTER}])`,
      'gi',
    ),
    (match) => sameCase('гродно', match),
  ],
  // Прилагательные: минский/минская/минском… → гродненский/гродненская/гродненском…
  [
    new RegExp(`(?<![${RU_LETTER}])минск(ий|ая|ое|ие|ого|ой|ому|ую|им|их|ими|ом)(?![${RU_LETTER}])`, 'gi'),
    (match, ending) => sameCase(`гродненск${ending.toLowerCase()}`, match),
  ],
];
