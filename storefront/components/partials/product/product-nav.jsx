      ...
      {product.next ? (
        <li className="product-nav-next">
          <ALink href={{ pathname: router.pathname, query: { slug: product.next.slug } }} scroll={false}>
            ...
            <span className="product-nav-popup">
              <img src={process.env.NEXT_PUBLIC_ASSET_URI + product.next.pictures[0].url} alt={(product.seo?.seoImage[index]?.imageAlt || product.name) + '-' + (index + 1)} title={(product.seo?.seoTitle || product.name) + '-' + (index + 1)} width="110" height="123" />
